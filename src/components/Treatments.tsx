      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-lg leading-relaxed text-neutral-500">
            Ofrecemos una amplia variedad de tratamientos realizados por
            especialistas en cada área.
          </p>
        </div>

        <div className="mx-auto mt-14 grid max-w-6xl gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {treatments.map((treatment) => (
            <TreatmentCard
              key={treatment.id}
              treatment={treatment}
              isOpen={openTreatment === treatment.id}
              onToggle={() =>
                setOpenTreatment(openTreatment === treatment.id ? null : treatment.id)
              }
            />
          ))}
        </div>
...
function TreatmentCard({
  ...
  return (
    <button
      type="button"
      onClick={onToggle}
      className={`group relative rounded-2xl border p-6 text-left transition-all hover:shadow-md ${
        isOpen
    ...
    >
      ...
      <p
        className={`mt-2 text-sm leading-relaxed text-neutral-500 transition-all ${
        ...
        <div className="mt-4 space-y-3 border-t border-brand-100 pt-4">
          ...
          <div className="flex flex-wrap gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-neutral-600">{treatment.specialist}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-brand-600">
                Duración
              </p>
              <p className="mt-1 text-xs text-neutral-600">{treatment.duration}</p>
            </div>
          </div>
          <a
            href={getWhatsAppUrl()}
            ...
            className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-xs font-semibold text-white transition-colors hover:bg-brand-700"
          >
